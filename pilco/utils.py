...
def plot_mountaincar_rollouts(steps,
    ...
    # Get true trajectories
    true_thetas = true_traj[:, 0]
    print(true_traj.shape, true_thetas.shape)
    true_thetadots = true_traj[:, 1]
    ...
    s_linspace = tf.cast(tf.linspace(-1.5, 2.5, s_points), dtype=tf.float64)

    s_dot_linspace = tf.cast(tf.linspace(-0.08, 0.08, s_dot_points), dtype=tf.float64)
